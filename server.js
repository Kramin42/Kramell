	...
	for (i=0; i<availablemissions.length; i++) {
		...
		if (account['missionqual'][mission].every(Boolean)) {//fully qualified
			...
			if (newmissions.length>1) {
				...
				promises.push(db.dieselrobin.update({'account': account['account']},{$set: {'currentmission': -1}}));
		...
		if (!account['bonuspoints'][i] && !account['bonusdisqual'][i]) {//check it's not done already or disqualified
			...
			if (i==0 && j==0) {//T1A
				if (milestone.search("br=Lair.*type=br.enter")>-1) {
					...
					promises.push(db.dieselrobin.update({'account': account['account']},{$set: {'bonusqual.0': true, 'bonuspoints.0': 3}}));
					announce=true; bonuswon=0; points=3;
				}
			}
			if (i==0 && j==1) {//T1B
				if (milestone.search("xl=(\d|1[012]):.*br=Lair.*type=br.end")>-1) {
					//account['bonusqual'][i]=true;
					promises.push(db.dieselrobin.update({'account': account['account']},{$set: {'bonusqual.0': true, 'bonuspoints.0': 3}}));
					announce=true; bonuswon=1; points=3;
				}
			}
			if (i==0 && j==2) {//T1C
				...
				if (milestone.search("br=(Swamp|Shoals|Spider|Snake|Slime).*type=br.enter")>-1) {
					...
					if (oldgod!=newgod && oldgod!='Ru' && !(oldgod.search("Ely|The Shining One|Zin")>-1 && newgod.search("Ely|The Shining One|Zin")>-1)) {
						promises.push(db.dieselrobin.update({'account': account['account']},{$set: {'bonusqual.0': [true,true], 'bonuspoints.0': 3}}));
						announce=true; bonuswon=2; points=3;
					}
				}
			}
			...
			if (i==2 && j==0) {//T3A
				if (milestone.search("urune=3")>-1) {
					...
					announce=true; bonuswon=6; points=8;
				}
			}
     		...
     		teams.forEach(function(team) {
     			...
     			scores.push(db.dieselrobin.find({'account': new RegExp('^('+team['accounts'].join('|')+')$','i')}).toArray().then(function(accounts) {
     				...
     				if (accounts && accounts[0]) {
						...
						potmissionscores = potmissionscores.sort(function(a, b){return b-a;});
						...
						return Promise.resolve({'team': team['team'], 'score': score, 'potscore': potscore});
     				}
     				//console.log('no points for team '+team['team']);
     				return Promise.resolve({'team': team['team'], 'score': 0, 'potscore': 100});
     			}));
     		});
